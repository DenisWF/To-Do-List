...
from PyQt6.QtWidgets import QApplication, QMainWindow, QWidget, QListWidget, QListWidgetItem, QVBoxLayout, QLabel, \
    QHBoxLayout, QPushButton, QMessageBox, QComboBox, QLineEdit, QTimeEdit
...
class TasksWindow(QWidget):
    ...
    def init_tasks(self, completed=False):
        ...
        try:
            ...
            if completed:
                select_query = """SELECT Nameing, Importance_task, Completed_task, Category 
                FROM Task 
                WHERE Dates = ? AND Completed_task = 'yes'"""
            else:
                select_query = """SELECT Nameing, Importance_task, Completed_task, Category 
                FROM Task 
                WHERE Dates = ?"""
        ...
        if selected_item:
            ...
            if reply == QMessageBox.StandardButton.Yes:
                try:
                    ...
                    delete_query = """DELETE FROM Task WHERE Nameing = ? AND Dates = ?"""
    ...
    def change_priority_color(self, event, current_circle):
        ...
        if selected_item:
            ...
            try:
                ...
                update_query = """UPDATE Task SET Importance_task = ? WHERE Nameing = ? AND Dates = ?"""
    ...
    def add_task_to_db(self):
        ...
        try:
            ...
            sqlite_insert_query = """INSERT INTO Task
                                             (Nameing, Importance_task, Completed_task, Category, Dates, Times)
                                             VALUES (?, ?, ?, ?, ?, ?);"""
...
class Redact_task(QWidget):
    ...
    def load_task_data(self):
        try:
            ...
            select_query = """SELECT Importance_task, Category, Completed, Times 
            FROM Task 
            WHERE Nameing = ? AND Dates = ?"""
    ...
    def save_task_changes(self):
        ...
        try:
            ...
            SET Nameing = ?, Importance_task = ?, Category = ?, Completed_task = ?, Times = ?
                              WHERE Nameing = ? AND Dates = ?"""
    ...
    def notification_loop(self):
        while self.running:
            ...
            try:
                ...
                query = """SELECT Nameing, Importance_task, Category
                           FROM Task 
                           WHERE Dates = ? AND Times = ? AND Completed_task = 'no'"""
